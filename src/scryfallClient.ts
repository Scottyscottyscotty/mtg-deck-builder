 ...
 * Checks for newcards-full.json (auto-generated) first, then newcards.json (manual)
 */
async function loadNewCardsFromFile(colorIdentity: string[], maxCards: number = 20): Promise<string> {
  try {
    const fs = await import('fs/promises');
    const path = await import('path');

    // Try newcards-full.json first (generated by update-new-cards.js)
    const fullPath = path.join(process.cwd(), 'newcards-full.json');
    const manualPath = path.join(process.cwd(), 'newcards.json');

    let filePath = manualPath;
    let useFullFormat = false;

    try {
      await fs.access(fullPath);
      filePath = fullPath;
      useFullFormat = true;
      console.log('📦 Using newcards-full.json (auto-generated bulk data)');
    } catch {
      console.log('📝 Using newcards.json (manual entry)');
    }
    ...
    try {
      ...
      if (useFullFormat) {
        // Full format has card objects with details
        summary += `The following cards from recent sets (${parsed.sets.slice(0, 2).map((s: any) => s.name).join(', ')}) may be relevant:\n\n`;

        for (const set of parsed.sets.slice(0, 2)) {
          if (!set.cards || set.cards.length === 0) continue;

          // Filter by color identity
          let filteredCards = set.cards;
          if (colorIdentity.length > 0) {
            filteredCards = set.cards.filter((card: any) => {
              if (!card.color_identity || card.color_identity.length === 0) {
                return true; // Colorless fits anywhere
              }
              return card.color_identity.every((c: string) => colorIdentity.includes(c));
            });
          }
          ...
          // Limit to maxCards per set
          const cardsToShow = filteredCards.slice(0, Math.floor(maxCards / 2));
...
export async function getNewCardsSummary(colorIdentity: string[], maxCards: number = 20): Promise<string> {
  try {
    ...
    if (latestSets.length === 0) {
      console.log('⚠️  Scryfall API unavailable, checking local card database...');
      return await loadNewCardsFromFile(colorIdentity, maxCards);
    }
